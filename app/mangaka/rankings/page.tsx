...
import { Series, PollData } from '@/lib/mock-data'
import {
  TrendingUp,
  TrendingDown,
  Minus,
...
export default function MangakaRankingsPage() {
  ...
  const [allSeries] = useState<Series[]>([])
  const [latestPoll] = useState<PollData | null>(null)
  ...
  return (
    <AppShell>
      <div className="space-y-6">
        ...
        <Card className="bg-card">
          <CardHeader>
            <CardTitle>Bảng xếp hạng tổng{latestPoll ? ` - Tuần ${latestPoll.weekNumber}/${latestPoll.year}` : ''}</CardTitle>
